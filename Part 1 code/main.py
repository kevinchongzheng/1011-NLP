...
def create_augmented_dataloader(args, dataset):
    ...
    # 1. Start from the *raw* train split (with "text" + "label")
    train_orig = dataset["train"]

    # 2. Sample 5,000 random indices from the training data
    num_aug = 5000
    total_n = len(train_orig)
    num_aug = min(num_aug, total_n)  # safety
    indices = random.sample(range(total_n), num_aug)

    sampled = train_orig.select(indices)

    # 3. Apply our custom transformation to the sampled examples
    sampled_transformed = sampled.map(custom_transform, load_from_cache_file=False)

    # 4. Concatenate original + transformed to get the augmented training set
    augmented_raw = datasets.concatenate_datasets([train_orig, sampled_transformed])

    # 5. Tokenize and prepare for the model (same steps as in __main__)
    augmented_tokenized = augmented_raw.map(tokenize_function, batched=True)
